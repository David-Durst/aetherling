...
def test_reduce_t_basic():
    ...
    tester.circuit.valid_up = 1
    for i in range(num_iterations*num_in+1):
        tester.print("clk: {}\n".format(i))
        if i < len(test_vals):
        ...
        if i >= num_in:
            tester.circuit.valid_down.expect(1)
        if i % num_in == 0 and i > 0:
            tester.circuit.O.expect(outputs[i // num_in - 1])
        tester.step(2)
    compile_and_run(tester)
...
def test_reduce_t_map_1_s():
    ...
    for i in range(num_iterations*num_in+1):
        ...
        if i >= num_in:
            tester.circuit.valid_down.expect(1)
        if i % num_in == 0 and i > 0:
            tester.circuit.O[0].expect(outputs[i // num_in - 1])
        tester.step(2)
    compile_and_run(tester)
...
def test_reduce_t_4_map_1_s_map_1_s():
    num_in = 4
    num_iterations = 2
    test_vals = [2,4,6,8,1,3,5,9]
    outputs = [20,18]
    ...
    for i in range(num_iterations*num_in + 1):
        ...
        if i >= num_in:
            tester.circuit.valid_down.expect(1)
        if i % num_in == 0 and i > 0:
            tester.circuit.O[0][0].expect(outputs[i // num_in - 1])
        tester.step(2)
    compile_and_run(tester)

def test_reduce_t_1_map_1_s_map_1_s():
    num_in = 1
    num_iterations = 3
    test_vals = [2,4,3]
    outputs = [2,4,3]
    red = wrap_module_with_top(DefineReduce_T(num_in, 0, DefineMap_S(1, DefineMap_S(1, DefineAdd_Atom(False), False), False)))
